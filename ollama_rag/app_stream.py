...
st.set_page_config(page_title="RAG Application Ollama",layout="centered")
st.title("Analyze PDF with ollama")
upload_files = st.file_uploader("Upload a PDF File",type=["PDF"],accept_multiple_files=True)

query = st.text_input("Ask Q")

if st.button("Submit") and upload_files is not None:
    if query:
        ...
        for file in upload_files:
            ...
            for page in reader.pages:
                all_data += page.extract_text() or ""
            st.warning(f'Extracted- {file.name}')

        # data = all_data
        documents_ = [Document(page_content=all_data)]
        ...
        documents = text_spllitter.split_documents(documents_)
